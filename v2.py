...
def load_data():
    products = []
    with open('data/amazon_products.csv', 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
    ...
    if 'compare' in tokens and 'price' in tokens:
        ...
        if product_name:
            ...
            for product in products:
                if product_name in product[0].lower():
                    matches.append(product)

            if len(matches) > 0:
                # Sort the matches by price
                matches.sort(key=lambda x: float(x[1].replace('$', '')))

                # Print the cheapest match
                cheapest = matches[0]
                return f"The cheapest {product_name} on Amazon is {cheapest[0]} at {cheapest[1]}"
